...
class TempAddActivity : AppCompatActivity() {
    private val TAG = javaClass.simpleName

    private lateinit var binding : ActivityStepAddDataBinding

    private lateinit var healthConnectManager: HealthConnectManager
    private val permissions = setOf(HealthPermission.getWritePermission(BodyTemperatureRecord::class))
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.deptAddTitle.text = "체온"
        binding.texttime.text ="시간"
        binding.textC.text = "온도"
        binding.dataUnitTV.visibility = View.VISIBLE
        binding.stepsEditText.inputType = InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_FLAG_DECIMAL
        ...
        binding.stepAddTimeCL2.visibility = View.GONE
        ...
        binding.addbutton.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                ...
                val value = binding.stepsEditText.text.toString().toDouble()
                try{
                    healthConnectManager.writeTempRecord(value, startTime)

                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@TempAddActivity, "Successfully insert records", Toast.LENGTH_SHORT).show()
                    }
                    finish()
                }
                catch (e: Exception) {
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@TempAddActivity, e.message.toString(), Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
